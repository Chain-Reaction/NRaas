...
{
    public class SimpleListDialog : ModalDialog
    {
        ...
        {
            return SimpleListDialog.Show(title, funds, listObjs, headers, viewTypeToggle, new Vector2(-1f, -1f));
        }
        ...
        public SimpleListDialog(string title, int funds, List<ObjectPicker.TabInfo> listObjs, List<ObjectPicker.HeaderInfo> headers, bool viewTypeToggle, Vector2 position)
            : base("SimplePurchaseDialog", 1, true, ModalDialog.PauseMode.PauseSimulator, null)
        {
            if (this.mModalDialogWindow != null)
            {
                ...
                this.mTable.Populate(listObjs, headers, listObjs.Count);
