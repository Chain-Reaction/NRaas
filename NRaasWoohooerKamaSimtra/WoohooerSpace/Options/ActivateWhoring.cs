...
{
    public class ActivateWhoring : OperationSettingOption<Sim>, ISimOption
    {
        bool mEnabled;

        protected override bool Allow(GameHitParameters< Sim> parameters)
        {
            if (!Skills.KamaSimtra.Settings.mShowRegisterInteraction) return false;
            ...
            if (!skill.CanWhore()) return false;
        ...
        public override string GetTitlePrefix()
        {
            if (mEnabled)
            {
                return "DisableWhoring";
            }
            else
            {
                return "EnableWhoring";
            }
        }
