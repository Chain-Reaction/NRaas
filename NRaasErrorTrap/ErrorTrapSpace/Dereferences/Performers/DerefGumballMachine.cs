...
{
    public class DerefGumballMachine : DereferenceGeneric<GameObject>
    {
        protected override bool Matches(string type)
        {
            return (type == "Sims3.Store.Objects.GumballMachine");
        }
        ...
        protected override DereferenceResult Perform(GameObject reference, FieldInfo field, List<ReferenceWrapper> objects)
        {
            ...
            if (Matches(reference, "mGumballUser", field, objects, out result, out actualField) != MatchResult.Failure)
            {
                if (result.Valid)
                {
