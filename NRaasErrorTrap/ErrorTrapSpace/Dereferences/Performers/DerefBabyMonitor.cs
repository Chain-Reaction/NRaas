...
{
    public class DerefBabyMonitor : DereferenceGeneric<GameObject>
    {
        protected override bool Matches(string type)
        {
            return (type == "Sims3.Store.Objects.BabyMonitor");
        }
        ...
        protected override DereferenceResult Perform(GameObject reference, FieldInfo field, List<ReferenceWrapper> objects)
        {
            ...
            if (Matches(reference, "mFakeAutonomyBaby", field, objects, out result, out actualField) != MatchResult.Failure)
            {
                if (result.Valid)
                {
