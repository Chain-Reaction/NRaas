...
{
    public class TraitMatchScoring : Scoring<SimDescription, DualSimScoringParameters>
    {
        int mHitScore = 0;
        int mMissScore = 0;

        public TraitMatchScoring()
        { }
        ...
        {
            int score = 0;
            foreach (Trait trait in parameters.Other.TraitManager.List)
            {
                if (trait.IsReward) continue;

                if (parameters.Actor.TraitManager.HasElement(trait.Guid))
                {
                    score += mHitScore;
                }
                else
