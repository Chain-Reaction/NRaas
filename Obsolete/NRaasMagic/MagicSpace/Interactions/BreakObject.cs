...
{
    public class BreakObject : Interaction<Sim, GameObject>, Common.IAddInteraction
    {
        ...
        {
            Target.Repairable.BreakObject(Actor, true);
            return true;
        }
        ...
        [DoesntRequireTuning]
        public sealed class Definition : InteractionDefinition<Sim, GameObject, BreakObject>
        {
            public override string GetInteractionName(Sim a, GameObject target, InteractionObjectPair interaction)
            {
                return Common.Localize("BreakObject:MenuName", a.IsFemale);
            }
            ...
            {
                if ((((target != a) && !(target is Sim)) && (!(target is Lot) && !(target is Sims3.Gameplay.Abstracts.RabbitHole))) && (!target.InUse && (target.Repairable != null)))
                {
                    if (!a.Inventory.ContainsType(typeof(MagicWand), 1))
                    {
                        return false;
                    }
                    if (a.SimDescription.ChildOrAbove && !target.Repairable.Broken)
                    {
                        if (Magic.GetSkillLevel(a) >= Magic.Settings.mBreakObjectLevel)
                        {
                            return true;
                        }
                    }
