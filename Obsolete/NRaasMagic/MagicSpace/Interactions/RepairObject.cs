...
{
    public class RepairObject : Interaction<Sim, GameObject>, Common.IAddInteraction
    {
        ...
        {
            Target.Repairable.ForceRepaired();
            return true;
        }
        ...
        [DoesntRequireTuning]
        public sealed class Definition : InteractionDefinition<Sim, GameObject, RepairObject>
        {
            public override string GetInteractionName(Sim a, GameObject target, InteractionObjectPair interaction)
            {
                return Common.Localize("RepairObject:MenuName", a.IsFemale);
            }
            ...
            public override bool Test(Sim a, GameObject target, bool isAutonomous, ref GreyedOutTooltipCallback greyedOutTooltipCallback)
            {
                if ((((target != a) && !(target is Sim)) && (!(target is Lot) && !(target is Sims3.Gameplay.Abstracts.RabbitHole))) && (!target.InUse && (target.Repairable != null)))
                {
                    ...
                    {
                        if (Magic.GetSkillLevel(a) >= Magic.Settings.mRepairObjectLevel)
                        {
                            return true;
                        }
                    }
