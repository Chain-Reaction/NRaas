...
{
    public class AddToddlerBooks : DebugEnablerInteraction<Bookshelf>
    {
        ...
        public override bool Run()
        {
            try
            {
                ...
                {
                    Target.Inventory.TryToAdd(BookToddler.CreateOutOfWorld(data));
                }
        ...
        [DoesntRequireTuning]
        private sealed class Definition : DebugEnablerDefinition<AddToddlerBooks>
        {
            public override string GetInteractionName(IActor a, Bookshelf target, InteractionObjectPair interaction)
            {
                return Common.Localize("AddToddlerBooks:MenuName");
            }
        }
    }
}