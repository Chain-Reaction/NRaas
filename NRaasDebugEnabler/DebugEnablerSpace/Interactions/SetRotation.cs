...
{
    public class SetRotation : DebugEnablerInteraction<GameObject>
    {
        ...
        public override bool Run()
        {
            try
            {
                string value = StringInputDialog.Show(Common.Localize("SetRotation:MenuName"), Common.Localize("SetRotation:Prompt"), "0");

                float rotation = 0;
                if (!float.TryParse(value, out rotation))
                {
        ...
        [DoesntRequireTuning]
        private sealed class Definition : DebugEnablerDefinition<SetRotation>
        {
            public override string GetInteractionName(IActor a, GameObject target, InteractionObjectPair interaction)
            {
                return Common.Localize("SetRotation:MenuName");
            }
        }
    }
}